...
namespace Bots
{
    ...
    public class OptimisedTreeNode
    {
        static double c = Math.Sqrt(2);
        public List<OptimisedTreeNode>? children;
        public OptimisedTreeNode? parent;
        ...
        public OptimisedTreeNode(SeededGameState fatherGameState, IHeuristic heuristic, Move? nodeMove, OptimisedTreeNode? fatherOrig, 
            List<Move> possibleMoves = null)
        {
            ...
            this.children = new List<OptimisedTreeNode>();
        }
        ...
        public void CreateChilds()
        {
            ...
            {
                this.children.Add(new OptimisedTreeNode(this.nodeGameState, stateEvalHeuristic, childMove, this));
            }
        }
        ...
        public OptimisedTreeNode SelectBestChild()
        {
            OptimisedTreeNode bestChild = this.children[0];

            double bestScore = 0;

            foreach (OptimisedTreeNode child in this.children)
            {
    ...
    public class OptimisedMCTSBot : AI
    {

        OptimisedTreeNode? actRoot;
        OptimisedTreeNode? actNode;
        bool startOfTurn;
        ...
        private IHeuristic stateEvaluationHeuristic = new OptimisedMCTSDefaultHeuristic();

        public OptimisedMCTSBot()
        {
            this.PrepareForGame();
        }
        ...
        private OptimisedTreeNode TreePolicy(OptimisedTreeNode v, SeededRandom rng)
        {
            if (v.children.Count() > 0)
            {
                ...
                int selectedChild = 0;
                int index = 0;
                foreach (OptimisedTreeNode childNode in v.children)
                {
        ...
        private void BackUp(OptimisedTreeNode? v, double delta)
        {
            if (v is not null)
            {
                v.visits += 1;
        ...
        public override Move Play(GameState gameState, List<Move> possibleMoves)
        {
            ...
            if (startOfTurn)
            {
                actRoot = new OptimisedTreeNode(sgs, stateEvaluationHeuristic, null, null, possibleMoves);
